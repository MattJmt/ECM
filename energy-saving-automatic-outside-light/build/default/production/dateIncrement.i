...
void dateIncrement(char* daylightSavingsP, char* dayOfWeekP, char* dayOfMonthP, char* monthP, unsigned int* yearP) {
    ...
    if (hour >= 24){
        ...
        *dayOfWeekP +=1;
        *dayOfMonthP += 1;
        if (runSyncro){syncro(daylightSavingsP);}
    }

    if (*dayOfWeekP > 7){*dayOfWeekP = 1; }


    if (*dayOfMonthP > 30 & (*monthP == 4 | *monthP == 6 | *monthP == 9 | *monthP == 11)){*dayOfMonthP = 1; *monthP +=1;}
    if (*dayOfMonthP > 31 & (*monthP == 1 | *monthP == 3 | *monthP == 5 | *monthP == 7 | *monthP == 8 | *monthP == 10)){*dayOfMonthP = 1; *monthP +=1;}
    if (*dayOfMonthP > 27 & *monthP == 2){
        if (*yearP%4 == 0){
            if (*dayOfMonthP > 29){*dayOfMonthP = 1; *monthP +=1;}
        }
        else{
            if (*dayOfMonthP > 28){*dayOfMonthP = 1; *monthP +=1;}
        }
    }

    if (*dayOfMonthP > 31 & *monthP == 12){*dayOfMonthP = 1; *monthP = 1; *yearP+=1;}


    if (*monthP == 3 & *dayOfMonthP > 24 & *dayOfWeekP == 7 & !*daylightSavingsP & hour == 1){hour -= 1; *daylightSavingsP = 1;}
    if (*monthP ==8 & *dayOfMonthP > 24 & *dayOfWeekP == 7 & *daylightSavingsP) {hour += 1; *daylightSavingsP = 0;}
