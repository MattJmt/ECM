...
void syncro(char daylightSavings) {
    char trueMid = 12;
    if (daylightSavings){char trueMid = 13;}
    if ( ((sunUp - sunDown) > 7) & (sunUp - sunDown < 16) ){
    midDay = ((sunUp + sunDown)/2);
        if (midDay != trueMid){

            if ((midDay - trueMid) > 0){hour += (midDay - trueMid);}

            else{
                while (hour < (trueMid - midDay)){_delay((unsigned long)((1)*(64000000/4000.0)));}
                hour = 0;
